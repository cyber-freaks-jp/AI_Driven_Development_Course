...
### ステップ2：AIに動作確認とエラー修正をさせる

実装が完了したら、**人間が手動で動作確認するのではなく、AIに自動で動作確認とエラー修正をさせます。**
...
2. curl コマンドで POST /api/payment/create-intent をテスト
3. エラーが出た場合は原因を特定して修正
4. 正常に動作するまで繰り返す
...
原因を分析します。
環境変数 STRIPE_SECRET_KEY が設定されていないことが原因です。
...
このように、**AIが自動でテスト→エラー修正→再テストを繰り返し、正常に動作するまで自動で修正してくれます。**
