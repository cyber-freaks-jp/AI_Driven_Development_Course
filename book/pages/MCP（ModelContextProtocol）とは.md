...
// 取得結果の例：
// [{ name: "add_calendar_event", description: "カレンダーに予定を追加する", parameters: {...} }]
```

**ステップ② 〈人間〉 自然言語で話しかける**

```
"明日14時に会議の予定を入れて。色は赤で"
```
...
```
ユーザー入力: "明日14時に会議の予定を入れて。色は赤で"
         ↓  ← AIが自動で解釈
使うツール  : add_calendar_event
パラメータ  : title="会議", date="2025-10-27", time="14:00", color="赤"
```
