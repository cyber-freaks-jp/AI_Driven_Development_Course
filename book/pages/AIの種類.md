...
これまでに、AIの基本的な仕組みを学びました。ここでは、AI駆動開発でよく耳にする「機械学習」「ディープラーニング」「生成AI」「LLM」といった用語について、それぞれの違いと関係性を整理していきます。
...
```mermaid
graph LR
    A[入力データ<br/>メールの内容] --> B[機械学習モデル]
    C[正解ラベル<br/>スパム or 正常] --> B
    B --> D[学習完了モデル]
    E[新しいメール] --> D
    D --> F[スパム判定結果]
```

例えば、スパムメール判定システムを作る場合、「これはスパム」「これは正常なメール」という正解ラベルが付いたメールデータを大量に用意し、モデルに学習させます。

**2. 教師なし学習**

正解ラベルなしで、データの中から特徴やグループを自動的に見つける方法です。
...
ゲームAIで使われることが多く、「良い行動をしたら報酬を与え、悪い行動をしたらペナルティを与える」という方法で学習します。例えば、囲碁AIのAlphaGoは、何百万回もの対局を通じて強化学習で学習しました。

### エンジニアの業務での活用例

機械学習は、すでに多くの開発現場で活用されています。

- **ログ分析**：システムログから異常パターンを検出
- **需要予測**：過去のデータから将来のアクセス数を予測し、サーバーリソースを最適化
- **レコメンドシステム**：ユーザーの行動履歴から、おすすめ商品を提案

## ディープラーニング：脳の仕組みを模倣した学習

ディープラーニング（Deep Learning）は、機械学習の一種です。前章で説明したように、人間の脳のニューラルネットワークを模倣した、多層のネットワーク構造を持っています。
...
プログラムコードを生成します。GitHub Copilot、Cursorなどが代表例です。

- コメントから関数を自動生成
- バグの修正提案
- テストコードの自動作成

AI駆動開発で使うツールの多くは、この「テキスト生成AI」と「コード生成AI」です。
...
```mermaid
graph LR
    A[大量のデータ] -->|学習フェーズ| B[AIモデル]
    C[プロンプト<br/>指示] -->|生成フェーズ| B
    B --> D[新しいコンテンツ]

    style B fill:#4dc3ff
```

**学習フェーズ**では、インターネット上の膨大なテキストデータ（ウェブサイト、書籍、コードなど）を読み込み、言葉の使い方やパターンを学習します。

**生成フェーズ**では、学習した知識をもとに、プロンプト（ユーザーからの指示）に応じて新しいコンテンツを生成します。

## LLM：言葉を理解する大規模モデル

LLM（Large Language Model：大規模言語モデル）は、生成AIの中でも特に「自然言語処理」に特化したモデルです。

### LLMの「大規模」とは

LLMの「大規模」という言葉には、2つの意味があります。

**1. パラメータ数が膨大**

LLMは、数十億から数千億のパラメータ（学習によって調整される値）を持っています。このパラメータが多いほど、複雑なパターンを学習できます。

例えば、GPT-3は1,750億個、GPT-4はさらに多くのパラメータを持っています。

**2. 学習データが膨大**

インターネット上の膨大なテキストデータ（数兆語以上）を学習しています。これにより、幅広い知識と言語パターンを獲得しています。

### LLMができること

LLMは、自然言語に関するさまざまなタスクをこなせます。
...
graph TD
    ...
    B --> E[質問の意図<br/>文章の意味<br/>コードの動作]
    C --> F[回答<br/>文章<br/>コード]
    D --> G[要約<br/>翻訳<br/>リファクタリング]
...
現在、AI駆動開発で広く使われているLLMには、以下のようなものがあります。

- **GPT-4（OpenAI）**：ChatGPT、GitHub Copilotのベース
- **Claude（Anthropic）**：長文の理解に優れる
- **Gemini（Google）**：Google製のLLM

これらのLLMを活用することで、エンジニアはコーディング、デバッグ、ドキュメント作成などの作業を大幅に効率化できます。
